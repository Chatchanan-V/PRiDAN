    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            for data in dataloaders[phase]:
                ...
                for k in range(now_batch_size):
                    ...
                    for kk in hard:
                        ...
                        if now_label != anchor_label:
                            top_n_negative.append(kk)
                            count = count + 1
                            if count == n:
                                break
                    #print('top_n_negative', top_n_negative)
                    #top_n_negative = torch.tensor(top_n_negative)
                    n_nf = nf_data[top_n_negative,:] #[3,2048]
                    n_nf_t = n_nf.transpose(0,1) #[2048,3]
                    f_anchor = f.data[k].reshape(1,-1)
                    score_anchor_negative = torch.mm(f_anchor, n_nf_t) #[1,3]
                    #print('score_anchor', score_anchor_negative)
                    weight_negative = torch.exp(score_anchor_negative)
                    weight_neg = weight_negative/torch.sum(weight_negative)
                    #print('weight_neg', weight_neg)
                    #print('weight_neg shape', weight_neg.shape)
                    
                    nscore[k] = torch.sum(score_anchor_negative*weight_neg)
                    #nscore = torch.tensor(nscore, requires_grad=True)
                
                # print('nscore', nscore)
                # print('nscore shape', nscore.shape)
                
                
                
                
                # nf_hard = torch.zeros(f.shape).cuda() # [16,2048]
                # for k in range(now_batch_size):
                #     hard = rank[k,:] # [64,]
                #     for kk in hard:
                #         now_label = neg_labels[kk] 
                ...
                # hard-pos
                # ----------------------------------
                pf_hard = torch.zeros(f.shape).cuda() # [16,2048]
                for k in range(now_batch_size):
                    pf_data = pf[4*k:4*k+4,:] # [4,2048]
                    pf_t = pf_data.transpose(0,1) # [2048,4]
                    ff = f.data[k,:].reshape(1,-1) # [1,2048]
                    score = torch.mm(ff, pf_t) #cosine # [1,4]
                    score, rank = score.sort(dim=1, descending = False) #score low == hard
                    pf_hard[k,:] = pf_data[rank[0][0],:] # [16,2048]

                iden_matrix = torch.eye(now_batch_size).cuda()
                torch1 = nscore.expand_as(iden_matrix)
                # loss
                # ---------------------------------
                criterion_triplet = nn.MarginRankingLoss(margin=opt.margin)                
                pscore = torch.sum( f * pf_hard, dim=1) # [16,]
                #nscore = nscore.clone()
                nscore = torch.sum( iden_matrix * torch1, dim=1) # [16,]
                
                #print('pscore', pscore)
                #print('pscore shape', pscore.shape)
                #print('nscore', nscore)
                #print('nscore shape', nscore.shape)
                ...
                if not opt.PCB:
                    ...
                    for i in range(num_part):
                        part[i] = outputs_pcb[i] #[16,781]

                    score = sm(part[0]) + sm(part[1]) +sm(part[2]) + sm(part[3]) +sm(part[4]) +sm(part[5])
                    _, preds = torch.max(score.data, 1)

                    loss_pcb = criterion(part[0], labels)
                    for i in range(num_part-1):
                        loss_pcb += criterion(part[i+1], labels)
                    #print('loss_pcb', loss_pcb)

                    #Global triplet identification
                    loss_global_triplet_iden = criterion(outputs,labels)
                    #print('loss_global_triplet_iden', loss_global_triplet_iden)

                    _, preds_triplet = torch.max(outputs.data, 1)
                    reg = torch.sum((1+nscore)**2) + torch.sum((-1+pscore)**2)
                    loss_triplet = torch.sum(torch.nn.functional.relu(nscore + opt.margin - pscore)) + opt.alpha*reg
                    #print('loss_triplet', loss_triplet)
                    
                    loss = loss_pcb + loss_triplet + loss_global_triplet_iden
                    #print('loss (loss_pcb + loss_triplet + loss_global_triplet_iden)', loss)
                ...
                if int(version[0])>0 or int(version[2]) > 3: # for the new version like 0.4.0 and 0.5.0
                    ...
                    running_loss += loss.data[0] #*opt.batchsize
